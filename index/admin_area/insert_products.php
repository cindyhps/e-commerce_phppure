...
if(isset($_POST['insert_product'])){
    ...
    $product_status = "true";
    $product_stock = $_POST['product_stock']; // Menyimpan jumlah stok dari form
    ...
    empty($product_image) || empty($product_stock)){
        ...
        $insert_products = "INSERT INTO products (product_title, product_description, product_keywords, category_id, brand_id, product_image, product_price, product_stock, date, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?)";

        if ($stmt = mysqli_prepare($con, $insert_products)) {
            mysqli_stmt_bind_param($stmt, "sssisssis", $product_title, $product_description, $product_keywords, $product_category, $product_brands, $product_image, $product_price, $product_stock, $product_status);
...
<form action="" method="post" enctype="multipart/form-data">
    <!-- Product Title -->
    <!-- Tambahan: Form untuk input stock -->
    ...
    <div class="form-outline mb-4 w-50 m-auto">
        ...
        <input type="number" name="product_stock" id="product_stock" class="form-control" placeholder="Enter Product's Stock" autocomplete="off" required>
    </div>
